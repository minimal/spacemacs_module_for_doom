...
error recovery."
  (load (concat dotspacemacs-template-directory
                ".spacemacs.template"))
  (advice-add 'dotspacemacs/layers
              :after
              (lambda (&rest _args)
                (setq-default dotspacemacs-install-packages 'used-but-keep-unused)
                (advice-remove 'dotspacemacs/layers 'error-recover-preserve-packages))
              '((name . error-recover-preserve-packages)))

  (advice-add 'dotspacemacs/init
              :after
              (lambda (&rest _args)
                              ...
                              (intern
                               (ido-completing-read
                                ...
                                '(("vim" vim)
                                  ...
                                  ("hybrid" hybrid))
                                nil t nil nil 'vim)))
                (advice-remove 'dotspacemacs/init 'error-recover-prompt-for-style))
              '((name . error-recover-prompt-for-style))))
